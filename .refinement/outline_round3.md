# Outline: IJF Submission (Round 3)

---

## Title Alternatives

Three options are proposed below; the author should select one.
All are 10-15 words and position "Causal" as interpretive.

Option A (13 words):
Learned Sparse Graph Attention for Forecast Combination:
Causal Interpretation in Oil Volatility

Option B (12 words):
Causally Interpretable Graph Attention Networks for
LLM Forecast Combination in Oil Volatility

Option C (11 words):
Graph Attention Forecast Combination with Causal Interpretation
for Oil Volatility

Recommended: Option B.
It preserves "Causally Interpretable" from Round 2, signals the
LLM context, names the core method (Graph Attention Networks),
and identifies forecast combination and the domain.
At 12 words it sits within the IJF norm of 10-15.
...
output, connecting to and extending the forecast combination
literature (Wang et al., 2023; Gibbs and Vasnev, 2024).
...
extractors: FinGPT (Liu et al., 2023), multi-dimensional WTI
sentiment analysis (Dai et al., 2026), and the FinBERT family for
...
that the optimal combination structure is time-varying and potentially
...
Machine learning: Ridge, Lasso, gradient boosting, and random
forest, all estimated on the same forward-looking target.
Deep learning: XGBoost, LSTM, and Transformer, estimated under
...
The learned sparse GAT achieves RMSE = 0.1226 on the 972
...
The central finding is that after HAC correction, the learned GAT
is the only model that is statistically significantly better than
HAR.
...
Among machine learning and deep learning baselines, the
best-performing model fails to reach conventional significance
thresholds after HAC correction [TO VERIFY from results CSVs:
insert DM_HAC and p-value from the model with lowest RMSE among
Ridge, Lasso, GBR, RF, XGBoost, LSTM, Transformer]; the full
statistics for all 13 models are in Table 1.
This result establishes that the information in the LLM forecasts
is real, but realising it requires the learned combination mechanism.
...
correction reverses this conclusion, a finding with direct
...
confirming that the graph structure contributes independently of
the LLM input quality.

Table 3 presents the ablation results, comparing graph structure vs
no-graph, debate vs single-agent input, and regime gate vs no
regime gate.

### 7.4 Regime-Conditional Analysis

Volatility regimes are defined by percentile cutoffs of the
persistence volatility: low (below 25th percentile, n = 65),
normal (25th to 75th percentile, n = 454), elevated (75th to 95th
percentile, n = 377), and high (above 95th percentile, n = 76).

The GAT dominates in the large-sample regimes.
In the normal regime, GAT achieves RMSE = 0.1310 versus MLP at
0.1552 and HAR at 0.1479.
In the elevated regime, GAT achieves RMSE = 0.1115 versus MLP at
0.1436 and HAR at 0.1362.
...
Table 4 presents the regime-conditional RMSE for GAT, MLP, HAR,
and naive debate across the four regimes.
...
However, no edge achieves unanimous agreement across seeds within
any single window; the highest individual edge frequency, observed
for the directed connection from the monetary agent to the
macro-demand agent, is 48.8%.
...
agents show the largest absolute contributions, consistent with
supply disruption episodes driving elevated volatility.
...
Herding is observed in 42.9% of agent-day observations across the
full evaluation sample, with technical and cross-market agents
herding most frequently at 52%.
...
error (partial r values below 0.08), confirming that the GAT has
already absorbed the herding signal through the behavioural encoding
...
Network combines them by discovering agent interaction structure
and adapting to volatility regimes.
Evaluated on 972 out-of-sample predictions with HAC-corrected DM
tests, the framework achieves RMSE = 0.1226 and is the only
...
quality (GAT vs MLP: DM_HAC = -2.825, p = 0.005), and the learned
...
confirming that the causal prior adds no information beyond what
the data-driven graph discovers, while potentially constraining
the search.
...
MLP no-graph, Learned sparse GAT.

Table 2: Pairwise HAC-corrected DM test matrix for six key models
(Persistence, HAR, Single, Debate-naive, MLP, Learned GAT).

Table 3: Ablation results (graph vs no-graph, debate vs single
input, regime gate vs no gate), all 5-seed ensemble,
HAC DM vs HAR.
...
learnable edge logit matrix, the dual-head structure, and the
regime gate.
...
The following references introduced or confirmed in this round
require full bibliographic details before submission.
